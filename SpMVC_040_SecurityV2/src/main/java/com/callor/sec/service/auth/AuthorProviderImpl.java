...
public class AuthorProviderImpl implements AuthenticationProvider {
	...
	/*
	 * Authentication
	 * 로그인 한 사용자의 정보를 전달받을 객체
	 * username / password를 통한 login 절차
	...
	public Authentication authenticate(Authentication authentication) throws AuthenticationException {
		...
		/*
		 * 최초 회원가입을 했을때는 아직 정상 절차가 완료되지 않아서 로그인을 성공해도
		 * 다른 기능을 사용 할 수 없도록 하기 위하여
		 * 회원 데이터의 isEnable 값을 false로 설정하고
		 * 이후 다른 절차를 통하여 정상 사용자임이 입증되면
		 * isEnalbled 칼럼을 true로 하여 사용을 인가하는 절차를 진행한다
		 */
		if (user.isEnabled() == false) {
			throw new BadCredentialsException(username + " : 가입 절차 미완료");
		}

		/* 
		 * 인증 완료 시 사용자 정보를 Security.Context token과 함께 담아서 로그인 성공을 프로젝트에 전파하기 위해 객체로 구현
		 * 
		 * user : 로그인 한 사용자 정보
		 * user.getAuthorities : 로그인 한 사용자의 권한 리스트
		 */
