...
public class BBsController {
	 ...
	 * BBsVO 타입의 bbsVO 객체 값이 필요하므로 inject 해달라는 요청
	 * 
	 * spring은 어딘가에 있는 @ModelAttribute("bbsVO")가 설정된 method를 찾아서 실행하여 return 값을 주입해준다
	 */
	...
	public String write(@ModelAttribute("bbsVO") BBsVO bbsVO, @RequestParam("up_file") MultipartFile file,
			Model model) {
...
//			bbsVO.setImages(fileDao.fineByBBsSeq(b_seq));
	...
	private BBsVO bbsVO() {
		...
		BBsVO bbsVO = BBsVO.builder().b_date(dayFormat.format(date)).b_time(timeFormat.format(date))
				.b_writer("jinwoo8420").build();
